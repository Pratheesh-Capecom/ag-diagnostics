...
<div class="row mb-3 video_url">
    ...
    <div class="col-5">
        @if(isset($feature))
            @if($feature->pdf)
            <img src="{{asset('/public/upload/media/feature/image').'/'.$feature->pdf}}" alt="No Image" id="image_tag" width="100" height="100">
            @endif
        @endif
    </div>
</div>
{{-- <div class="row mb-3">
    <label class="col-2 text-end col-form-label">PDF Upload</label>
    <div class="col-10">
        {!! Form::file('file[]', ['class' => 'form-control', "id"=>"attachment", 'autocomplete' => 'off','accept'=>"application/pdf","multiple"]) !!}
    </div>
</div> --}}
{{-- <div class="col-md-6">
    <p id="files-area">
...
{{-- @if(isset($feature))
    @foreach(json_decode($feature['pdf']) as $key=>$val)
        @if($val)
        <div class="col-md-6">
            <ul>
                <li>{{$val}} <a> <i class="fa-solid fa fa-trash" onclick="deletePdf({{ $key }})"></i></a></li>
            </ul>
        </div>
