<div class="d-flex flex-column flex-shrink-0 text-white   side_bar" style="width: 280px;">
    <a href="{{ route('admin.dashboard') }}" class="d-flex shadow p-3 border-light align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none" style="border-bottom: 1px solid #ffffff24 !important">
        <img src="{{ asset('/public/images/logo/logo.png') }}" alt="logo" width="90%" class="mx-auto">
    </a>
    <ul class="nav nav-pills flex-column mb-auto mt-3 pt-0 p-3">
       ...
       @if($user->hasAccess('user.view.category')||$user->hasAccess('user.add.category')||$user->hasAccess('user.edit.category'))
        ...
        <li>
            <a href="{{ route('manage.index') }}" class="nav-link text-white {{ Route::is(['manage.index','manage.create','manage.edit']) ? "active" : "" }}">
        ...
        <li>
            <a href="{{ route('manage_package.index') }}" class="nav-link text-white {{ Route::is(['manage_package.index','manage_package.create','manage_package.edit']) ? "active" : "" }}">
        ...
        <li>
            <a href="{{ route('country.index') }}" class="nav-link text-white {{ Route::is(['master.index','branch.show','area.index','area.create','area.edit','city.index','city.create','city.edit','country.index','country.create','country.edit','state.index','state.create','state.edit', 'banner.edit', 'test.edit' , 'banner.create', 'banner.index']) ? "active" : "" }}">
        ...
        <li>
            <a href="{{ route('home_visit.index') }}" class="nav-link text-white {{ Route::is(['home_visit.index','book_test.index','contact_us.index','admin_careers.index','enquiry_request_call.index','enquiry_package.index']) ? "active" : "" }}">
